...
def map_file2dataset(user, filepath, decrypted_checksum, dataset_id):
    ...
    conn2 = psycopg2.connect(user='lega_out',
    ...
    with conn2.cursor() as cursor:
        for f in files:
            cursor.execute('INSERT INTO local_ega_ebi.filedataset(id, file_id, dataset_stable_id) VALUES(%(last_index)s, %(file_id)s, %(dataset_id)s)',
                           {"last_index": last_index + 1 if last_index is not None else 1, "file_id": f[0], "dataset_id": dataset_id})
            last_index += 1
...
def retrieve_file2dataset():
    """Assign file to dataset, for dataset driven permissions."""
    conn = psycopg2.connect(user='lega_out',
                            password=os.environ.get('DB_LEGA_OUT_PASSWORD'),
                            database='lega',
                            host=os.environ.get('DB_HOST', 'localhost'),
                            sslmode='require',
